...
use super::{CloneFresh, algorithm::Compress};
...
impl<C: Compress> CompressFrame<C> {
    ...
    /// It can be accessed using [`frame`](Self::frame).
    ///
    /// The caller must pass a `raw` payload no larger than `u32::MAX`, because the compressed
    /// frame stores the original payload length in a 32-bit field.
