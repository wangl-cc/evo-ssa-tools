...
use super::{CacheStore, StorageError, StorageNamespace, StorageResult};
...
/// `redb` backend root for managed persistent cache spaces.
#[derive(Debug, Clone)]
...
impl RedbBackend {
    /// Open a redb database root for managed cache namespaces.
    pub fn open(path: impl AsRef<Path>) -> StorageResult<Self> {
        let db = redb::Database::create(path)?;
        Ok(Self { db: Arc::new(db) })
    }

    /// Create a backend root from an owned database handle.
    pub fn from_database(db: redb::Database) -> Self {
        Self { db: Arc::new(db) }
    }

    /// Create a backend root from a shared database handle.
    pub fn from_database_arc(db: Arc<redb::Database>) -> Self {
        Self { db }
    }
}
...
impl PersistentBackend for RedbBackend {
    ...
    fn open_namespace(&self, namespace: &StorageNamespace) -> Result<Self::Store> {
        let store = RedbStore::from_database_arc(self.db.clone(), namespace.as_str())?;
        Ok(store)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_redb_backend_opens_distinct_namespaces() -> CrateResult<()> {
        use crate::cache::{ManagedPersistentCache, provider::CacheProvider};

        let file = tempfile::NamedTempFile::new().unwrap();
        let backend = RedbBackend::open(file.path())?;
        let provider = ManagedPersistentCache::new(backend, FixtureEngine::default());
    ...
    #[test]
    fn test_redb_backend_from_database_opens_namespace() -> CrateResult<()> {
        let file = tempfile::NamedTempFile::new().unwrap();
        let db = ::redb::Database::create(file.path()).map_err(StorageError::from)?;
        let backend = RedbBackend::from_database(db);
        let path = ComputationPath::root(ComputationId::new("from-database/v1"));
        let namespace = StorageNamespace::new(&path, ValueFormat::new("raw/v1"));
        let store = backend.open_namespace(&namespace)?;

        store.store_encoded(b"k", b"payload")?;
        let encoded = store.fetch_encoded(b"k")?.expect("value should exist");
